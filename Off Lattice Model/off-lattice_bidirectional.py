#!/usr/bin/env python3
"""
Simulates bidirectional crowd flow dynamics with panic effects (normal scenario).

Implements off-lattice agent-based model with social forces.
Tracks density, speed, and panic levels over time.
Generates animations, snapshots, and population dynamics plots.
...
def plot_average_speed(times, avg_speeds):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('average_speed_evolution.png', dpi=150)
    plt.show()
...
def plot_individual_snapshots(snapshots):
    ...
    for idx, (t_snap, pos_snap, dir_snap) in enumerate(snapshots):
        ...
        plt.tight_layout()
        plt.savefig(f'snapshot_{idx + 1}_t_{t_snap:.0f}s.png', dpi=150)
        plt.close(fig)  # Close figure to free memory


def main():
    # Run unified simulation once
    sim_params = copy.deepcopy(params)
    start_time = time.time()
    results = run_unified_simulation(sim_params)
    print(f'Unified simulation took {((time.time() - start_time) / 60.0):.2f} minutes with dt = {params["dt"]}')
    ...
    # Generate all plots from the collected data
    plot_average_speed(times, avg_speeds)
    plot_individual_snapshots(snapshots)
    create_animation(anim_pos, anim_dirs, anim_times, anim_n_agents)
    plot_population_model(pop_t, pop_I, pop_N, pop_P, pop_V)


def create_animation(positions_history, directions_history, times_history, n_agents_history):
    ...
    ani_ax.set_title('Normal Crowd Flow Simulation')
    ani_ax.grid(True)
    ...
    ani.save('bidirectional_crowd_flow2.mp4', writer=writer, dpi=100)


def plot_population_model(t, I_eff, N_sim, P_sim, V_sim):
    ...
    plt.tight_layout()
    plt.savefig('offlattice_crowd_dynamics.png', dpi=300)
    plt.show()


if __name__ == '__main__':
    # Unified simulation took 16.66 minutes with dt = 0.01
    # Unified simulation took 3.61 minutes with dt = 0.05
    main()