#!/usr/bin/env python3
"""
Lattice-based crowd simulation with panic dynamics (normal scenario).

Implements cellular automaton model with:
- Directional movement preferences
- Density-dependent panic generation
- Conflict resolution for agent movement
- Time-varying inflow conditions

Visualizes agent positions, panic fields, and flow metrics.
...
def move_agents(grid, panic, density, params):
    ...
    # Resolve conflicts
    for target, agents in move_plans.items():  # move_plans :
        ni, nj = target
...
def plot_time_series(times, densities, panics, num_agents, grid_history, params):
    ...
    plt.tight_layout()
    plt.savefig('lattice_time_series.png', dpi=300)
    plt.show()


def create_animation(grid_history, panic_history, params, fps=5):
    ...
    plt.tight_layout()
    plt.savefig('lattice_simulation_snapshot.png', dpi=300)
    plt.show()

    # Save animation
    writer = animation.FFMpegWriter(fps=fps, bitrate=5000)
    ani.save('lattice_crowd_animation.mp4', writer=writer)
    debug("Animation saved as 'lattice_crowd_animation.mp4'")

    return ani


def plot_grid_snapshot(grid, panic, params, time=None, filename=None):
...
def plot_spatial_distribution(grid_history, panic_history, params):
    ...
    plt.tight_layout()
    plt.savefig('lattice_spatial_distribution.png', dpi=300)
    plt.show()


def plot_flow_rate(times, grid_history, params):
    ...
    plt.tight_layout()
    plt.savefig('lattice_flow_rate.png', dpi=300)
    plt.show()


def main():
    # Create a copy of parameters for safety
    sim_params = copy.deepcopy(params)

    # Run simulation
    debug("Starting lattice simulation...")
    start_time = time.time()
    times, densities, panics, grid_history, panic_history = run_simulation(
        sim_params, record_interval=1.0
    )
    debug(f'Simulation completed successfully!, time {((time.time() - start_time) / 60.0):.2f} minutes with dt = {params["dt"]}', True)

    # Calculate agent counts for each frame
    num_agents = [np.sum(grid != 0) for grid in grid_history]

    # Time series plot
    debug("Creating visualizations...")
    plot_time_series(times, densities, panics, num_agents, grid_history, params)

    # Grid snapshot at peak time
    peak_idx = np.argmax(num_agents)
    plot_grid_snapshot(
        grid_history[peak_idx],
        panic_history[peak_idx],
        params,
        time=times[peak_idx],
        filename='lattice_grid_snapshot.png'
    )

    # Spatial distribution plots
    plot_spatial_distribution(grid_history, panic_history, params)

    # Flow rate analysis
    plot_flow_rate(times, grid_history, params)

    # Create animation
    debug("Creating animation...")
    create_animation(grid_history, panic_history, params, fps=10)
